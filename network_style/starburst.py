...
LINE_WIDTH = 2                                      ## > 0, int - Width of connecting lines.
REDNESS = 1                                         ## 0 to 1 - reddishness of final pattern
BLUENESS = 1                                        ## 0 to 1 - bluishness of final pattern
GREENNESS = 1                                       ## 0 to 1 - greenishness of final pattern
...
class output(object):
    ...
    def display(self, program):
        ...
        self.draw_nodes(program)
        self.draw_connections(program, program.parent_node)
    ...
    def draw_connections(self, program, parent_node):

        if(parent_node.children):
            for child in parent_node.children:
                pygame.draw.line(self.screen, parent_node.color, parent_node.position, child.position, LINE_WIDTH)
                self.draw_connections(program, child)                               ## Recursively draw connections of each child.
